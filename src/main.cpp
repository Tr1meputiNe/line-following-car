...
// ---- 速度（0~255）----
// 【调这几个数的办法】电机装在 Arduino 的 5V 引脚上，那颗自恢复保险丝的"保持电流"是 500mA。
// 只要两个电机的总电流超过它，保险丝就发热、阻值暴涨、电压塌到 2.2V，单片机复位。
// 所以从低往高试：先跑 85，看待机画面的 minV 稳不稳在 5V 附近；
//   稳 -> 每次加 10 往上试，找到"刚好不塌"的最高速度（跑得快一点成绩也好看）
//   不稳 -> 再往下减
// 原来 130 实测直接塌到 2.2V，所以起点定在 85。
int speedFast = 85;      // 直行速度（两个轮子一样）
int speedTurn = 90;      // 转弯时【外侧】轮速度
int speedBack = 30;      // 转弯时【内侧】轮【反转】速度；改成 0 = 内侧轮只停住不倒转，最省电
