...
// ---- 速度（0~255，就是 PWM 占空比）----
// 【本次：取前后两次的中位数】
//   speedFast  试过 130 和 110  -> 中位数 120
//   speedTurn  试过 155 和 130  -> 中位数 142（142.5 取整）
//   speedBack  试过 100 和  80  -> 中位数  90
// 也就是"比原来慢、比刚调的略快"的中间档。这个档位兼顾两件事：
//   速度低一点 -> 转向/过线不容易冲过头，电机电流也小（实测占空比 130 时 5V 轨被拉到 2.30V）
//   速度高一点 -> 过弯之后能及时走出来，不至于慢到在弯里磨蹭
// 之后要调就继续用这个办法：每次在前一次和这一次之间取中位数，很快就能收敛。
int speedFast = 120;    // 直行速度（两个轮子一样）
int speedTurn = 142;    // 转弯时【外侧】轮速度，越大转得越急
int speedBack = 90;     // 转弯时【内侧】轮【反转】速度，越大转得越急
